...
const actions = {
  processMessageExt({ commit, state }, msg) {
    ...
    if (Array.isArray(msg)) {
      msg.map((msgItem, idx) => {
        try {
          ...
          if (newHash !== '' && newHash !== currentHash && msgItem.time > currentMsgTime) {
    ...
    if (!userId) return
    const currentHash = state.userProfiles.get(userId)?._meta?.lastMessageHash || ''
    const currentMsgTime = state.userProfiles.get(userId)?._meta?.lastMessageTimestamp || 0
    const newHash = generateMsgHash(msg)
    // 哈希值不同时且消息晚于更新过的消息才提交更新
    if (newHash !== currentHash && newHash !== '' && msg.time > currentMsgTime) {
