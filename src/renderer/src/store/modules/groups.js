...
const Groups = {
  ...
  mutations: {
    ...
    SET_GROUP_MEMBERS_INFO: (state, payload) => {
      ...
      inGroupInfo.length > 0 &&
        inGroupInfo.map((item) => (groupMemberInfo = Object.assign(groupMemberInfo, item)))
      if (!state.groupDetails.has(groupId)) {
    ...
    UPDATE_CACHE_GROUP_INFO: (state, payload) => {
      ...
      if (type === 'groupName') {
        ...
        state.groupDetails.has(groupId) && (state.groupDetails.get(groupId).name = params)
      }
      //更新群组详情内的数据
      if (type === 'groupDescription') {
        ...
        state.groupDetails.has(groupId) && (state.groupDetails.get(groupId).description = params)
      }
      //更新群成员数
    ...
    UPDATE_GROUP_MEMBERS: (state, payload) => {
      ...
      switch (type) {
        case GROUP_OPERATION_TYPE.MEMBER_PRESENCE:
          {
            state.groupMembers.has(groupId) && state.groupMembers.get(groupId).push({ member })
          }
          break
        case GROUP_OPERATION_TYPE.MEMBER_ABSENCE:
          {
            if (state.groupMembers.has(groupId) && state.groupMembers.get(groupId).length > 0) {
              const _index = state.groupMembers.get(groupId).findIndex((item) => item === member)
    ...
    UPDATE_GORUPS_ADMIN: (state, payload) => {
      ...
      if (type === GROUP_OPERATION_TYPE.SET_ADMIN) {
        state.groupDetails.has(groupId) && (state.groupDetails.get(groupId).adminlist = [userId])
      } else if (type === GROUP_OPERATION_TYPE.REMOVE_ADMIN) {
        if (
          state.groupDetails.has(groupId) &&
  ...
  actions: {
    //从服务端获取加入的群组列表
    fetchJoinedGroupListFromServer: async ({ state, dispatch, commit }, params = {}) => {
      const {
    ...
    //批量获取群成员群内群组属性
    fetchGroupMemberAttributesFromServer: async ({ dispatch, commit }, params) => {
  ...
  getters: {
    ...
    getGroupName: (state) => (groupId) => {
      const group = state.joinedGroup.joinedGroupList.find((item) => item.groupId === groupId)
